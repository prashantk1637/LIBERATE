...
<div id="nav">
	...
	<%} %>
	<li class="nav-item"><a href="search.jsp">Book Search</a></li>
...
<%
	...
	String sql="select mem_id, librarian_id, return_date, fine_paid from returned";
	int flag=0;
	int total_fine=0;
	if(start_date==""&&end_date=="")
		 response.sendRedirect("librarian.jsp?fine_stmt=Please choose atleast one of from Start date or end date");
	PreparedStatement pstmt;
	Connection con;
	SimpleDateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	 SimpleDateFormat df1 = new SimpleDateFormat("yyyy-MM-dd");
	 ArrayList<ReturnStatusBean> fine_list=new ArrayList<>();
	try {
		...
		{	 Date date = df.parse(rs.getString(3));
		 	...
		 	if(end_date==""&&start_date!=null)
			{
				if(df1.parse(start_date).compareTo(df1.parse(reg_date_db))<=0)
				{
					...
					obj.setLibrarian_id(rs.getString(2));
					obj.setReturn_date(rs.getString(3));
					obj.setFine_paid(rs.getInt(4));
					total_fine=total_fine+rs.getInt(4);
					fine_list.add(obj);
				}
...
%>
<h2>Total Fine Collected Statement</h2>
...
<%for(ReturnStatusBean o: fine_list){ %>
<%if(o.getFine_paid()>0){ %>
<tr>
<td><%=o.getMem_id() %></td>
<td><%=o.getLibrarian_id()%></td>
<td><%=o.getReturn_date()%></td>
<td><%=o.getFine_paid()%></td>
</tr>
<%} %>
...
<td></td>
<td></td>
<td><b>Total Fine Collected</b></td>
<td><b>Rs.&nbsp;<%=total_fine %></b></td>
</tr>
</table>
